...
export const hasActiveChild = (item, currentUrl) => {
  ...
  for (const child of children) {
    ...
    // if(child.hasDynamicId)
   
    // Check if the child has a link and is active
    if (child && child.navLink && currentUrl && (child.navLink === currentUrl || currentUrl.includes(child.navLink) || currentUrl.includes(child.otherLink) || currentUrl.includes(child.extraLink)) )  {
      return true
    }
  }
...
export const canViewMenuItem = item => {
  return true
  // const ability = useContext(AbilityContext)
  // console.log(ability.can(item.action, item.resource),item.action, item.resource)
